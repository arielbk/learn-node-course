...
var mapOptions = {
  center: { lat: 43.2, lng: -81.1 },
  zoom: 10
};

function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 43.2;
  var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -81.2;
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // show details of place on marker click
...
function makeMap(mapDiv) {
  if (!mapDiv) return; // no div? skip it!
  //  make our map
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        ...
        // tell them an empty response came back
  ...
  searchInput.on('keyup', function (e) {
    // if not up down or enter, who cares!
